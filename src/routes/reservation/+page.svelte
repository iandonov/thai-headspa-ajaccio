...
<div class="min-h-screen bg-[--color-cream] pt-24 pb-16">
	<div class="max-w-4xl mx-auto px-6">
		...
		<div class="text-center mb-12">
			<p class="section-subheading">Sur Rendez-vous</p>
			<h1 class="section-heading">Réserver un Soin</h1>
				...
				<div class="flex items-center">
					<div class="flex flex-col items-center">
						<div class="w-9 h-9 rounded-full flex items-center justify-center text-sm font-sans font-medium transition-all duration-300
							{step >= s.n ? 'bg-[--color-forest] text-white' : 'bg-[--color-sand] text-[--color-stone]'}">
							{#if step > s.n}
						...
						<span class="font-sans text-xs mt-1 {step >= s.n ? 'text-[--color-forest]' : 'text-[--color-stone]/60'}">{s.label}</span>
					</div>
					{#if s.n < 3}
						<div class="w-16 h-px mx-3 mt-[-12px] {step > s.n ? 'bg-[--color-forest]' : 'bg-[--color-sand]'}"></div>
					{/if}
				</div>
			{/each}
		</div>
		...
		<form method="POST">
			...
			{#if step === 1}
				<div class="bg-white rounded-[--radius-card] shadow-[--shadow-soft] p-8">
