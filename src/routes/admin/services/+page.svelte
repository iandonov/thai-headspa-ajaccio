...
<div class="mb-8">
	...
	<p class="font-sans text-sm text-(--color-stone) mt-1">
		Gérez vos prestations à la carte et vos formules. Le champ « Lits » indique combien de lits/tables la prestation occupe.
	</p>
</div>
...
<section class="mb-12">
	<div class="flex items-center justify-between mb-4">
		<div>
			<h2 class="font-serif text-2xl text-(--color-charcoal)">Prestations à la carte</h2>
			<p class="font-sans text-xs text-(--color-stone)">Soins individuels proposés à l'unité</p>
		</div>
		<button type="button" onclick={() => (showAddBase = !showAddBase)}
			class="px-4 py-2 bg-(--color-forest) text-white text-xs font-sans rounded-sm hover:bg-(--color-forest-light) transition-colors">
	...
	{#if showAddBase}
		<form method="POST" action="?/createBase" use:enhance={() => async ({ update }) => { await update(); showAddBase = false; }}
			class="bg-(--color-sand)/20 rounded-(--radius-card) border border-(--color-sand) p-6 mb-4 grid grid-cols-1 sm:grid-cols-4 gap-4">
...
<section>
	<div class="flex items-center justify-between mb-4">
		<div>
			<h2 class="font-serif text-2xl text-(--color-charcoal)">Formules</h2>
			<p class="font-sans text-xs text-(--color-stone)">Forfaits personnalisables avec options au choix</p>
		</div>
